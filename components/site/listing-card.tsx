...
export function ListingCard({ business, featured = false }: { business: Business; featured?: boolean }) {
  const isFeatured = featured || business.tier === 'sponsored';
  const enhanced = !isFeatured && (business.tier === 'enhanced' || business.tier === 'featured');
  const specialties = specialtyList(business.specialties);

  return <article className={`relative flex h-full flex-col overflow-hidden rounded-2xl border bg-white p-5 shadow-[0_8px_35px_rgba(15,35,60,.06)] ${isFeatured ? 'border-[#ec7d2c] ring-2 ring-[#ec7d2c]/10' : enhanced ? 'border-[#42617e] ring-1 ring-[#142c4c]/10' : 'border-slate-200'}`}>
    ...
    {(business.phone || business.address || business.service_area || business.hours || specialties.length > 0 || business.year_founded || business.license_number) ? <dl className="mt-5 space-y-3 border-t border-slate-100 pt-4 text-sm text-slate-600">
      ...
      {business.year_founded ? <div className="flex items-start gap-2"><CalendarDays aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-[#d96c20]"/><dt className="sr-only">Year founded</dt><dd><span className="font-semibold text-slate-700">Founded:</span> {business.year_founded}</dd></div> : null}
      {business.license_number ? <div className="flex items-start gap-2"><Hash aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-[#d96c20]"/><dt className="sr-only">License number</dt><dd><span className="font-semibold text-slate-700">License:</span> {business.license_number}</dd></div> : null}
    </dl> : null}
