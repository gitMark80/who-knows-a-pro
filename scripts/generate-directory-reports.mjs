...
const inviteRows = [['business_name', 'email', 'city', 'category', 'main_profile_url', 'claim_url']];
for (const business of directorySeed) {
  if (!business.publicEmail) continue;
  const region = activeRegions.find((item) => item.slug === business.region);
  const trade = trades.find((item) => item.slug === business.trade);
  inviteRows.push([business.name, business.publicEmail, region?.name ?? business.location, trade?.name ?? business.trade, `https://whoknowsapro.com/business/${business.mainSlug}`, `https://whoknowsapro.com/claim?business=${business.id}`]);
}
